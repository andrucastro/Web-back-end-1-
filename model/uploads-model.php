...
function checkExistingImage($imgName){
   ...
   //Get Thumbnails based in the vehicle ID (display on datil veiw) 
   function getVehiclesImgThumbnails($invId){
    $db = phpmotorsConnect();
    $sql ="SELECT images.imgPath, inventory.invMake, inventory.invModel FROM images JOIN inventory ON images.invId = inventory.invId WHERE imgId IN (SELECT imgId  FROM images WHERE invId = :invId) AND imgPath LIKE '%-tn%'";
    $stmt = $db->prepare($sql);
    $stmt->bindValue(':invId', $invId, PDO::PARAM_INT);
    $stmt->execute();
    $thumPath = $stmt->fetchAll(PDO::FETCH_ASSOC);
    $stmt->closeCursor();
    return $thumPath;  
  }
